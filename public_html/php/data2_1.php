...
$query = sprintf("SELECT ROUND(SUM(potencia)*(0.62378769 ),2) AS preco,"
        . " hora, "
        . "concat(`dia`,'-',`mes`,'-',`ano`) as data "
        . "from medidasHora "
        . "where (date_add(str_to_date((concat(`ano`,',',`mes`,',',`dia`,' ',`hora`)),'%Y,%m,%d %H'), interval 1 hour))>=(date_sub(now(), interval 1 day)) "
        . "group by dia and hora");
