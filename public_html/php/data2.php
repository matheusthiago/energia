...
$query = sprintf("SELECT dia, ROUND(SUM(potencia)*(0.62378769 ),2) AS preco, hora, concat(`ano`,'-','0',`mes`,'-',`dia`), curdate() from medidasHora where concat(`ano`,'-','0',`mes`,'-',`dia`)=curdate() group by hora");
$SELEresult = $mysqli->query($query);
